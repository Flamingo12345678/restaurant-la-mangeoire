...
class PaymentManager {
    ...
    public function executePayPalPayment($payment_id, $payer_id) {
        try {
            ...
            $commande_id = null;
            if ($payment_info) {
                $commande_id = $payment_info['CommandeID'] ?? null;
                // Récupérer le client_id via la commande
                $client_id = $this->getClientIdFromPayment($payment_info);
            ...
            // Essayer de récupérer l'ID de commande même en cas d'erreur
            $commande_id = null;
            try {
                $payment_info = $this->getPaymentByTransactionId($payment_id);
                $commande_id = $payment_info['CommandeID'] ?? null;
    ...
    /**
     * Récupère le client_id à partir des informations de paiement
     */
    private function getClientIdFromPayment($payment_info) {
        if (!$payment_info || !$payment_info['CommandeID']) {
            return null;
        }
        
        try {
            $query = "SELECT ClientID FROM Commandes WHERE CommandeID = ?";
            $stmt = $this->pdo->prepare($query);
            $stmt->execute([$payment_info['CommandeID']]);
            $result = $stmt->fetch(PDO::FETCH_ASSOC);
            return $result ? $result['ClientID'] : null;
        } catch (Exception $e) {
            error_log("Get client ID from payment error: " . $e->getMessage());
            return null;
        }
    }
