...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  if (!check_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    log_admin_action('Tentative CSRF ajout table');
  } else {
    $numero = trim($_POST['numero'] ?? '');
    $places = intval($_POST['places'] ?? 0);
    // Validation stricte
    if (validate_length($numero, 1, 10) && $places > 0) {
      try {
        $sql = "INSERT INTO TablesRestaurant (Numero, Places) VALUES (?, ?)";
        $stmt = $conn->prepare($sql);
        $result = $stmt->execute([$numero, $places]);
        if ($result) {
        ...
        handle_pdo_exception($e, 'ajout table');
        log_admin_action('Erreur PDO ajout table', 'PDOException');
      }
    } else {
