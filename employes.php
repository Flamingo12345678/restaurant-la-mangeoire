...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['ajouter'])) {
  if (!check_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    if ($nom && $prenom && $poste && $salaire > 0 && $date_embauche) {
      ...
      set_message('Employé ajouté.', 'success');
    } else {
...
if (isset($_GET['delete'])) {
  if (!isset($_GET['csrf_token']) || !check_csrf_token($_GET['csrf_token'])) {
    ...
    $stmt->execute([$id]);
    set_message('Employé supprimé.', 'success');
  }
}
  ...
  <table border="1" cellpadding="5">
    ...
    <?php foreach ($employes as $e): ?>
      <tr>
        ...
        <td><a href="?delete=<?= $e['EmployeID'] ?>&csrf_token=<?= $csrf_token ?>" onclick="return confirm('Supprimer cet employé ?')">Supprimer</a></td>
      </tr>
    <?php endforeach; ?>
  </table>
</body>
