  ...
  if (!check_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    try {
      ...
      if ($result) {
        ...
        header('Location: commandes.php?msg=supprime');
        exit;
      } else {
      ...
      handle_pdo_exception($e, 'suppression commande');
...
<body style="background:#f7f7f7; min-height:100vh;">
  <div class="admin-nav">Administration – Supprimer une commande</div>
  <div class="form-container">
    ...
    <?php if ($id > 0): ?>
      <form method="post">
        ...
        <p>Êtes-vous sûr de vouloir supprimer cette commande (ID <?= e($id) ?>) ?</p>
        <button type="submit" class="delete-btn">Confirmer la suppression</button>
        <a href="commandes.php" class="back-link">Annuler</a>
      </form>
    <?php else: ?>
      <div class="alert alert-error">ID invalide.</div>
      <a href="commandes.php" class="back-link">Retour</a>
    <?php endif; ?>
  </div>
</body>
